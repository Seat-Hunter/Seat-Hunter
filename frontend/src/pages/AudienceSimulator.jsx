...
function getColors(audienceType, audienceIndex) {
  ...
  const p = CHAR_PROFILES[audienceIndex % CHAR_PROFILES.length];
  return {
    skin: p.skin, 
    shirt: shirts[audienceIndex % shirts.length],
    tieColor: ties[audienceIndex % ties.length], 
    hairColor: p.hairColor,
    hair: p.hair, 
    glasses: p.glasses, 
    glassesStyle: p.glassesStyle, 
    gender: p.gender,
  };
}
...
function makeHairFront(style, hc, skin) {
  switch(style) {
    case 'short':
      return `<path d="M8 14 Q8 1 20 1 Q32 1 32 14 Q27 8 20 8 Q13 8 8 14Z" fill="${hc}"/><rect x="7.5" y="12" width="4.5" height="4" rx="1.5" fill="${hc}"/><rect x="28" y="12" width="4.5" height="4" rx="1.5" fill="${hc}"/><path d="M8.5 11 Q14 11 20 9 Q26 11 31.5 11Z" fill="${hc}"/>`;
    case 'long':
      return `<path d="M8 14 Q8 1 20 1 Q32 1 32 14 Q27 8 20 8 Q13 8 8 14Z" fill="${hc}"/><rect x="6.5" y="11" width="5.5" height="30" rx="2.5" fill="${hc}"/><rect x="28" y="11" width="5.5" height="30" rx="2.5" fill="${hc}"/><path d="M8.5 11 Q15 11 18 11 Q22 11 31.5 11Z" fill="${hc}"/><path d="M7 34 Q7 40 9 44" stroke="${hc}" stroke-width="4" stroke-linecap="round" fill="none"/><path d="M33 34 Q33 40 31 44" stroke="${hc}" stroke-width="4" stroke-linecap="round" fill="none"/>`;
    case 'bob':
      return `<path d="M8 14 Q8 1 20 1 Q32 1 32 14 Q27 8 20 8 Q13 8 8 14Z" fill="${hc}"/><rect x="6.5" y="11" width="5.5" height="20" rx="2.5" fill="${hc}"/><rect x="28" y="11" width="5.5" height="20" rx="2.5" fill="${hc}"/><path d="M7.5 11 Q20 11 32.5 11Z" fill="${hc}"/><rect x="6.5" y="29" width="27" height="4" rx="2" fill="${hc}"/>`;
    case 'medium':
      return `<path d="M8 14 Q8 1 20 1 Q32 1 32 14 Q27 8 20 8 Q13 8 8 14Z" fill="${hc}"/><rect x="6.5" y="11" width="5.5" height="24" rx="2.5" fill="${hc}"/><rect x="28" y="11" width="5.5" height="24" rx="2.5" fill="${hc}"/><path d="M8 11 Q14 11 20 10 Q26 11 32 11Z" fill="${hc}"/>`;
    case 'ponytail':
      return `<path d="M8 14 Q8 1 20 1 Q32 1 32 14 Q27 8 20 8 Q13 8 8 14Z" fill="${hc}"/><rect x="7.5" y="12" width="4.5" height="5" rx="1.5" fill="${hc}"/><rect x="28" y="12" width="4.5" height="5" rx="1.5" fill="${hc}"/><path d="M8 11 Q13 11 20 11 Q27 11 32 11Z" fill="${hc}"/><ellipse cx="20" cy="3" rx="6" ry="2.5" fill="${hc}"/><rect x="18" y="0.5" width="5" height="18" rx="2.5" fill="${hc}"/>`;
    case 'wavy':
      return `<path d="M8 14 Q8 1 20 1 Q32 1 32 14 Q27 8 20 8 Q13 8 8 14Z" fill="${hc}"/><rect x="6.5" y="11" width="5.5" height="16" rx="2.5" fill="${hc}"/><rect x="28" y="11" width="5.5" height="16" rx="2.5" fill="${hc}"/><path d="M8 11 Q20 11 32 11Z" fill="${hc}"/><path d="M7 26 Q5 32 8 38 Q6 42 8 46" stroke="${hc}" stroke-width="5" stroke-linecap="round" fill="none"/><path d="M33 26 Q35 32 32 38 Q34 42 32 46" stroke="${hc}" stroke-width="5" stroke-linecap="round" fill="none"/>`;
    case 'curly':
      return `<ellipse cx="20" cy="5" rx="13" ry="8" fill="${hc}"/><circle cx="9" cy="9" r="5.5" fill="${hc}"/><circle cx="31" cy="9" r="5.5" fill="${hc}"/><circle cx="13" cy="5" r="4.5" fill="${hc}"/><circle cx="27" cy="5" r="4.5" fill="${hc}"/><circle cx="20" cy="2" r="4" fill="${hc}"/><path d="M9 11 Q14 11 20 11 Q26 11 31 11Z" fill="${hc}"/>`;
    default:
...
function makeGlassesFront(style) {
  if(style==='round') return `<circle cx="15.5" cy="14" r="4.2" fill="none" stroke="#222" stroke-width="1.8"/><circle cx="24.5" cy="14" r="4.2" fill="none" stroke="#222" stroke-width="1.8"/><line x1="19.7" y1="14" x2="20.3" y2="14" stroke="#222" stroke-width="1.8"/><line x1="11.3" y1="13.5" x2="9.5" y2="13" stroke="#222" stroke-width="1.5" stroke-linecap="round"/><line x1="28.7" y1="13.5" x2="30.5" y2="13" stroke="#222" stroke-width="1.5" stroke-linecap="round"/>`;
  return `<rect x="11" y="10.5" width="8" height="6.5" rx="1.5" fill="none" stroke="#222" stroke-width="1.8"/><rect x="21" y="10.5" width="8" height="6.5" rx="1.5" fill="none" stroke="#222" stroke-width="1.8"/><line x1="19" y1="13.8" x2="21" y2="13.8" stroke="#222" stroke-width="1.8"/><line x1="11" y1="13.5" x2="9.5" y2="13" stroke="#222" stroke-width="1.5" stroke-linecap="round"/><line x1="29" y1="13.5" x2="30.5" y2="13" stroke="#222" stroke-width="1.5" stroke-linecap="round"/>`;
}
...
function makeSideHair(style, hc) {
  switch(style) {
    case 'short':
      return `<path d="M4 16 Q3 1 20 1 Q31 1 33 11 Q34 14 32 16 Q29 9 24 7 Q18 6 14 8 Q10 11 11 16Z" fill="${hc}"/><path d="M4 15 Q4 24 13 25 Q10 21 11 16 Z" fill="${hc}"/><path d="M22 4 Q33 5 32 10 Q28 8 22 7 Z" fill="${hc}"/>`;
    case 'long': case 'medium':
      return `<path d="M4 16 Q3 1 20 1 Q31 1 33 11 Q34 14 32 16 Q29 9 24 7 Q18 6 14 8 Q10 11 11 16Z" fill="${hc}"/><path d="M4 15 Q3 ${style==='medium'?'35':'45'} 13 ${style==='medium'?'36':'46'} Q11 ${style==='medium'?'28':'38'} 11 16Z" fill="${hc}"/><path d="M22 4 Q33 5 32 11 Q28 8 22 7 Z" fill="${hc}"/>`;
    case 'bob':
      return `<path d="M4 16 Q3 1 20 1 Q31 1 33 11 Q34 14 32 16 Q29 9 24 7 Q18 6 14 8 Q10 11 11 16Z" fill="${hc}"/><path d="M4 15 Q3 30 13 32 Q10 26 11 16Z" fill="${hc}"/><path d="M4 30 Q12 32 21 32 Q13 33 4 31Z" fill="${hc}"/><path d="M22 4 Q33 5 32 10 Q28 8 22 7 Z" fill="${hc}"/>`;
    case 'ponytail':
      return `<path d="M4 16 Q3 1 20 1 Q31 1 33 11 Q34 14 32 16 Q29 9 24 7 Q18 6 14 8 Q11 12 16Z" fill="${hc}"/><path d="M4 15 Q4 24 13 25 Q10 21 11 16 Z" fill="${hc}"/><rect x="2" y="4" width="5" height="24" rx="2.5" fill="${hc}"/><path d="M22 4 Q33 5 32 10 Q28 8 22 7 Z" fill="${hc}"/>`;
    case 'wavy':
      return `<path d="M4 16 Q3 1 20 1 Q31 1 33 11 Q34 14 32 16 Q29 9 24 7 Q18 6 14 8 Q10 11 11 16Z" fill="${hc}"/><path d="M4 15 Q2 26 5 34 Q3 40 5 46 Q10 38 11 16Z" fill="${hc}"/><path d="M22 4 Q33 5 32 11 Q28 8 22 7 Z" fill="${hc}"/>`;
    case 'curly':
      return `<circle cx="14" cy="5"    r="6" fill="${hc}"/><circle cx="22" cy="3"    r="5.5" fill="${hc}"/><circle cx="29" cy="6"    r="5.5" fill="${hc}"/><circle cx="33" cy="11"  r="5" fill="${hc}"/><circle cx="9"  cy="10"  r="5.5" fill="${hc}"/><circle cx="7"  cy="17"  r="5" fill="${hc}"/><path d="M4 15 Q3 24 12 25 Q10 20 11 16Z" fill="${hc}"/><path d="M21 5 Q31 7 30 11 L25 8Z" fill="${hc}"/>`;
    default:
...
function makeSideGlasses(style) {
  if(style==='round') return `<circle cx="28" cy="15" r="4" fill="none" stroke="#222" stroke-width="1.6"/><line x1="32" y1="14.5" x2="34.5" y2="14" stroke="#222" stroke-width="1.3" stroke-linecap="round"/>`;
  return `<rect x="23" y="12" width="9" height="6" rx="1.3" fill="none" stroke="#222" stroke-width="1.6"/><line x1="32" y1="14.5" x2="34.5" y2="14" stroke="#222" stroke-width="1.3" stroke-linecap="round"/>`;
}

function makeTieFront(tieColor) {
  const dk = shadeColor(tieColor, -30);
  return `<polygon points="18,23 22,23 22.5,27 20,28.5 17.5,27" fill="${dk}"/><polygon points="17.6,27 22.4,27 21.2,37 20,38.5 18.8,37" fill="${tieColor}"/>`;
}

function makeTieSide(tieColor) {
  const dk = shadeColor(tieColor, -30);
  return `<polygon points="21,24 24,24 24,28 22.5,29 21,28" fill="${dk}"/><polygon points="21.2,28 24,28 23.5,38 22.5,39 21.5,38" fill="${tieColor}"/>`;
}
...
function makeSVG(mood, c, ch, delay=0, hasDesk=true, scale=1) {
  ...
  if(isCold) {
    eyes = `<circle cx="${ELX}" cy="${EY}" r="2" fill="#333"/><circle cx="${ERX}" cy="${EY}" r="2" fill="#333"/><line x1="${ELX-3}" y1="${EY-2}" x2="${ELX+2.5}" y2="${EY-2}" stroke="#333" stroke-width="1.3" stroke-linecap="round"/><line x1="${ERX-2.5}" y1="${EY-2}" x2="${ERX+3}" y2="${EY-2}" stroke="#333" stroke-width="1.3" stroke-linecap="round"/>`;
    mouth = `<path d="M16 20 Q20 18.5 24 20" stroke="#666" stroke-width="1.3" fill="none" stroke-linecap="round"/>`;
  } else if(isApplause) {
    eyes = `<path d="M${ELX-3} ${EY} Q${ELX} ${EY-3} ${ELX+3} ${EY}" stroke="#333" stroke-width="2" fill="none" stroke-linecap="round"/><path d="M${ERX-3} ${ERX-3} ${EY} Q${ERX} ${EY-3} ${ERX+3} ${EY}" stroke="#333" stroke-width="2" fill="none" stroke-linecap="round"/>`;
    mouth = `<path d="M14 19.5 Q20 25 26 19.5" stroke="#333" stroke-width="1.5" fill="none" stroke-linecap="round"/>`;
    brow = `<path d="M13.5 11 Q${ELX} 9.5 18.5 11" stroke="#555" stroke-width="1.1" fill="none" stroke-linecap="round"/><path d="M21.5 11 Q${ERX} 9.5 26.5 11" stroke="#555" stroke-width="1.1" fill="none" stroke-linecap="round"/>`;
  } else if(isRaising) {
    eyes = `<circle cx="${ELX}" cy="${EY}" r="2.2" fill="#222"/><circle cx="${ELX+0.5}" cy="${EY-0.8}" r="0.7" fill="white"/><circle cx="${ERX}" cy="${EY}" r="2.2" fill="#222"/><circle cx="${ERX+0.5}" cy="${EY-0.8}" r="0.7" fill="white"/>`;
    mouth = `<ellipse cx="20" cy="20.5" rx="2.5" ry="1.8" fill="#C62828"/>`;
    brow = `<path d="M13.5 10.5 Q${ELX} 9 18.5 10.5" stroke="#444" stroke-width="1.2" fill="none" stroke-linecap="round"/><path d="M21.5 10.5 Q${ERX} 9 26.5 10.5" stroke="#444" stroke-width="1.2" fill="none" stroke-linecap="round"/>`;
  } else if(isBored) {
    eyes = `<path d="M${ELX-2.5} ${EY-0.5} Q${ELX} ${EY+2} ${ELX+2.5} ${EY-0.5}" stroke="#aaa" stroke-width="1.8" fill="none" stroke-linecap="round"/><path d="M${ERX-2.5} ${EY-0.5} Q${ERX} ${EY+2} ${ERX+2.5} ${EY-0.5}" stroke="#aaa" stroke-width="1.8" fill="none" stroke-linecap="round"/>`;
    mouth = `<path d="M16.5 20 Q20 20.8 23.5 20" stroke="#ccc" stroke-width="1.2" fill="none" stroke-linecap="round"/>`;
  } else if(isConfused) {
    eyes = `<circle cx="${ELX}" cy="${EY}" r="2" fill="#444"/><circle cx="${ERX}" cy="${EY}" r="2" fill="#444"/>`;
    mouth = `<path d="M16 20 Q18 22 21 19.5 Q23 18.5 25 20" stroke="#666" stroke-width="1.3" fill="none" stroke-linecap="round"/>`;
    brow = `<path d="M14 11.5 Q${ELX} 10.5 18.5 11.5" stroke="#666" stroke-width="1" fill="none" stroke-linecap="round"/><path d="M21.5 11 Q${ERX} 12 26.5 11" stroke="#666" stroke-width="1" fill="none" stroke-linecap="round"/>`;
  } else if(isPhone) {
    eyes = `<circle cx="${ELX}" cy="${EY+1}" r="1.8" fill="#555"/><circle cx="${ERX}" cy="${EY+1}" r="1.8" fill="#555"/>`;
    ...
    eyes = `<circle cx="${ELX}" cy="${EY}" r="2.2" fill="#222"/><circle cx="${ELX+0.6}" cy="${EY-0.7}" r="0.7" fill="white"/><circle cx="${ERX}" cy="${EY}" r="2.2" fill="#222"/><circle cx="${ERX+0.6}" cy="${EY-0.7}" r="0.7" fill="white"/>`;
    mouth = `<path d="M15.5 19.5 Q20 23.5 24.5 19.5" stroke="#444" stroke-width="1.3" fill="none" stroke-linecap="round"/>`;
    brow = `<path d="M14 11.5 Q${ELX} 10.5 18.5 11.5" stroke="#555" stroke-width="1" fill="none" stroke-linecap="round"/><path d="M21.5 11.5 Q${ERX} 10.5 26.5 11.5" stroke="#555" stroke-width="1" fill="none" stroke-linecap="round"/>`;
  } else {
    eyes = `<circle cx="${ELX}" cy="${EY}" r="2" fill="#333"/><circle cx="${ELX+0.5}" cy="${EY-0.6}" r="0.6" fill="white"/><circle cx="${ERX}" cy="${EY}" r="2" fill="#333"/><circle cx="${ERX+0.5}" cy="${EY-0.6}" r="0.6" fill="white"/>`;
    mouth = `<path d="M16 19.5 Q20 23 24 19.5" stroke="#555" stroke-width="1.3" fill="none" stroke-linecap="round"/>`;
    brow = `<path d="M14 11.5 Q${ELX} 10.5 18.5 11.5" stroke="#777" stroke-width="0.9" fill="none" stroke-linecap="round"/><path d="M21.5 11.5 Q${ERX} 10.5 26.5 11.5" stroke="#777" stroke-width="0.9" fill="none" stroke-linecap="round"/>`;
  }

  const zzz = isBored ? `<text class="zzz-1" x="27" y="10" font-size="7" fill="#90A4AE" font-weight="500">z</text><text class="zzz-2" x="31" y="6"  font-size="9" fill="#90A4AE" font-weight="500">z</text><text class="zzz-3" x="35" y="2"  font-size="11" fill="#90A4AE" font-weight="500">z</text>` : '';
  ...
  if(isCold) {
    ...
    armL=`<rect x="${ALX-2}" y="${BY+2}" width="5" height="9" rx="2.5" fill="${c.shirt}"/><circle cx="${ALX}" cy="${BY+12}" r="3.5" fill="${c.skin}"/>`;
  } else if(isRaising) {
  ...
  const earring = isFemale ? `<circle cx="9.5" cy="17.5" r="1.5" fill="#E8C84A"/><circle cx="30.5" cy="17.5" r="1.5" fill="#E8C84A"/>` : '';
  const deskHTML = hasDesk && ch.desk ? `<rect x="0" y="42" width="40" height="18" rx="2" fill="${ch.desk}"/><rect x="0" y="40" width="40" height="4" rx="1" fill="${ch.deskTop}"/>` : '';
  ...
  return `<svg class="mood-${mood}" width="${w}" height="${h}" viewBox="0 0 40 60" fill="none">
    ...
    ${isRaising?'':(isApplause?`<rect x="${ARX+2}" y="${BY+2}" width="5" height="9" rx="2.5" fill="${c.shirt}"/><circle cx="${ARX+4}" cy="${BY+12}" r="3.5" fill="${c.skin}"/>`:
      isPhone?`<rect x="${ARX}" y="${BY+1}" width="5" height="7" rx="2.5" fill="${c.shirt}"/><rect x="${ARX-1}" y="${BY+5}" width="7.5" height="10" rx="1.5" fill="#2a2a2a"/>`:
    ...
    ${armL}
    ${isRaising?`<rect x="${ARX}" y="${BY-10}" width="5" height="15" rx="2.5" fill="${c.shirt}"/><circle cx="${ARX+2.5}" cy="${BY-11}" r="3.5" fill="${c.skin}"/>` : ''}
...
function makeSideSVG(mood, c, ch, dir='right', scale=1) {
  ...
  if(isCold) {
    eyeSVG=`<circle cx="${EX}" cy="${EY}" r="1.8" fill="#333"/><line x1="${EX-2.5}" y1="${EY-2}" x2="${EX+2}" y2="${EY-2}" stroke="#333" stroke-width="1.1" stroke-linecap="round"/>`;
    ...
    eyeSVG=`<circle cx="${EX}" cy="${EY}" r="1.9" fill="#333"/><circle cx="${EX+0.5}" cy="${EY-0.6}" r="0.6" fill="white"/>`;
  ...
  if(isApplause) {
    armOver=`<rect x="22" y="18" width="5" height="14" rx="2.5" fill="${c.shirt}"/><circle cx="24.5" cy="17" r="3.5" fill="${c.skin}"/>`;
  } else if(isRaising) {
    armOver=`<rect x="25" y="9" width="5" height="18" rx="2.5" fill="${c.shirt}"/><circle cx="27.5" cy="8" r="3.5" fill="${c.skin}"/>`;
  } else if(isCold) {
    armFront=`<rect x="10" y="${BY+2}" width="20" height="5" rx="2.5" fill="${shadeColor(c.shirt,-20)}"/>`;
  } else if(isPhone) {
    armOver=`<rect x="26" y="24" width="5" height="11" rx="2.5" fill="${c.shirt}"/><rect x="24" y="22" width="8" height="6" rx="1.5" fill="#2a2a2a"/>`;
  } else {
    armFront=`<rect x="28" y="${BY}" width="5" height="12" rx="2.5" fill="${c.shirt}"/>`;
  }

  const zzz = isBored ? `<text class="zzz-1" x="34" y="10" font-size="6" fill="#90A4AE">z</text><text class="zzz-2" x="37" y="7"  font-size="8" fill="#90A4AE">z</text>` : '';
...
function makeEmptySeatSVG(ch, hasDesk=true, scale=1) {
  const deskHTML = hasDesk&&ch.desk ? `<rect x="0" y="42" width="40" height="18" rx="2" fill="${ch.desk}" opacity="0.55"/><rect x="0" y="40" width="40" height="4" rx="1" fill="${ch.deskTop}" opacity="0.55"/>` : '';
  const w=Math.round(54*scale), h=Math.round(68*scale);
  return `<svg width="${w}" height="${h}" viewBox="0 0 40 60" fill="none">
    <rect x="5" y="26" width="30" height="14" rx="3" fill="${ch.back}" opacity="0.45"/><rect x="9" y="38" width="22" height="5" rx="2" fill="${ch.back}" opacity="0.3"/>
    ${deskHTML}
  </svg>`;
}
...
// ⚠️ 기존의 무작위 변조 shuffle 함수는 더 이상 자리 배치에 개입하지 않습니다.
...
export default function AudienceSimulator({ roomType='classroom', audienceType='general', count=8, memberMoods=[] }) {
  ...
  // 🔥 [리액트 메모리 박제] 방마다 입장 시 무작위 자리 배치 순서를 딱 1번만 고정시키는 상태창고
  const [fixedSeatsMap, setFixedSeatsMap] = useState({});

  useEffect(() => {
    // 1. 대형 강의실과 시청각실의 최대 좌석수 정의
    const classroomTotal = 6 * 3;     // 18자리
    const audiovisualTotal = 4 * 5;   // 20자리

    // 2. 처음 맵이 생성되거나 방이 리셋될 때, 절대로 리렌더링 시 뒤바뀌지 않는 고정 고유 셔플 시퀀스를 기억시킵니다.
    setFixedSeatsMap({
      classroom: shuffle([...Array(classroomTotal).keys()]),
      audiovisual: shuffle([...Array(audiovisualTotal).keys()])
    });
  }, [roomType]); // 방 모드가 바뀔 때만 한 번 난수를 생성해 박제합니다.

  const render = useCallback(() => {
    const area = areaRef.current;
    if (!area) return;

    // 언제나 버그 없는 깨끗한 캔버스 초기화
    area.innerHTML = '';
    ...
    if (roomType === 'audiovisual') {
      ...
      const seatSeq = fixedSeatsMap.classroom || [...Array(18).keys()];
      renderClassroom(area, colors, ch, count, memberMoods, seatSeq);
    } else if (roomType === 'meeting') {
      renderMeeting(area, colors, ch, memberMoods);
    }
  }, [roomType, audienceType, count, memberMoods, fixedSeatsMap]);
...
function renderAudiovisual(area, colors, ch, count, moods, seatSeq) {
  ...
  const ROWS=4, COLS=5;
  const occupied = new Set(seatSeq.slice(0, count));

  // 💡 좌석 배치 순서와 무관하게, 특정 학생이 기분 데이터 배열의 인덱스를 안정적으로 물고 있도록 보장합니다.
  const seatToMemberMap = {};
  let currentMemberIdx = 0;
  seatSeq.forEach(seatId => {
    if (occupied.has(seatId)) {
      seatToMemberMap[seatId] = currentMemberIdx++;
    }
  });
  ...
  for(let r=0; r<ROWS; r++){
    ...
    for(let c=0; c<COLS; c++){
      ...
      if(occupied.has(s)){
        const mIdx = seatToMemberMap[s];
        div.className = 'sim-member-wrap';
        div.innerHTML = `<div class="sim-svg-slot" style="overflow:visible; display:flex; align-items:center;">${makeSVG(moods[mIdx]||'neutral', colors[mIdx % colors.length], ch, 0, false, 0.95)}</div>`;
      } else {
...
function renderClassroom(area, colors, ch, count, moods, seatSeq) {
  ...
  for(let r=0; r<ROWS; r++){
    ...
    for(let c=0; c<COLS; c++){
      ...
      if(occupied.has(s)){
        const mIdx = seatToMemberMap[s];
        div.className = 'sim-member-wrap';
        div.innerHTML = `<div class="sim-svg-slot" style="overflow:visible; display:flex; align-items:center;">${makeSVG(moods[mIdx]||'neutral', colors[mIdx % colors.length], ch, 0, true, 0.95)}</div>`;
      } else {
        div.className = 'sim-member-wrap empty-seat';
